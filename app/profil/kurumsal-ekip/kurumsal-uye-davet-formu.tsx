...
import { inviteUserToTeam } from '@/app/kurumsal/business-actions';
import {
  MEMBER_ROLE_OPTIONS,
...
export function KurumsalUyeDavetFormu({ onClose }: Props) {
  ...
  const [memberRole, setMemberRole] = useState<BusinessMemberRole>('member');
    ...
    return (
      <div className="bg-[#1E3A5F]/5 border-2 border-[#1E3A5F]/15 rounded-lg p-6 text-center">
        ...
        <p className="text-sm text-ink">
          {email} adresine davet ulaştırıldı. Davet edilen kişi Kashe&apos;de
          kayıtlıysa bildirim alır, değilse email ile davet edilir.
        </p>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3 pt-1">
          ...
          <button
            type="submit"
            disabled={isPending}
            className="flex-1 inline-flex items-center justify-center gap-2 px-5 py-3 bg-brand-ink text-paper rounded-lg font-display font-semibold text-sm hover:-translate-x-0.5 hover:-translate-y-0.5 hover:shadow-[4px_4px_0_var(--color-ink)] disabled:opacity-50 disabled:cursor-not-allowed transition-all"
          >
